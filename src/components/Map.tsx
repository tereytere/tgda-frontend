...
import { MapContainerWrapper } from "../styledComponents/MapStyles";
...
export default function Map() {
	...
	const handleLegendItemClick = (legendItem: string) => {
		setSelectedLegendItem(
			legendItem === selectedLegendItem ? null : legendItem
		);
	};
		...
		<MapContainerWrapper ref={mapContainerRef}>
			<MapContainer center={defaultCenter} zoom={13} scrollWheelZoom={false}>
				...
				{markerDataList.map((markerData: MarkerData, index: number) => {
					if (
						selectedLegendItem &&
						markerData.markerType !== selectedLegendItem
					) {
						return null;
					}
				...
				<div className="legend">
					<div
						...
						onClick={() => handleLegendItemClick("store")}
					>
					...
					<div
						className="legend-item"
						onClick={() => handleLegendItemClick("restaurant")}
					>
					...
					<div
						className="legend-item"
						onClick={() => handleLegendItemClick("organization")}
					>
